...
(defun mapping-structural-evaluation (mapping parameters &optional (global?))
  ...
  (cond ((not (or (and global? (block-most-out-of-mapping-contributions? parameters))
         ...
         (setf (score mapping)
           (fset:reduce #'(lambda (mh sum) (+ mh sum))
			(mhs mapping)
...
(defun mhs-include-commutative-children (mapping)
  (let ((new-mhs)
        (old-mhs (mhs mapping))
        (possible-mhs))
    (fset:do-set (mh old-mhs (append possible-mhs new-mhs))
      (push mh new-mhs)
      (when (commutative-mh? mh)
        (fset:do-set (possible-mh (possible-descendants mh))
          (if (and (not (fset:contains? old-mhs possible-mh))
